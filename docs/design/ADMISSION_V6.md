...
the function's intention. Production still runs `cov_halflife_bars=720`, so the second half of the
drawdown result is not yet shipped — the measured study holds expected maximum drawdown at 10.2%
only with **both** changes, and neither alone suffices.
