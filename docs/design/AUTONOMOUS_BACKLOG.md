...
- `2026-08-22 18:20` — **F3 DONE. Track F complete, and with it every track in this file.**
  `docs/design/SYSTEM_MAP.md` — 467 lines, **generated by `scripts/build_system_map.py`, never
  written by hand**. A hand-written map is accurate on the day it is written and quietly wrong
  afterwards, which is worse than no map, because a reader trusts it.
  Everything is read off the files. Each script's purpose is **the first line of its own
  docstring**, so a script with no docstring appears in the map as having none — visible rather
  than absent. Each pipeline's steps are the scripts it **actually invokes**, in order, parsed from
  the shell. Each contract's boundary is **the boundary it states about itself**. The nine timed
  jobs come from the launchd agents installed on this machine, with what each runs and when.
  At a glance today: 192 Python scripts, 17 shell entry points, 6 configuration contracts, 14
  engineering artifacts, 236 unit test files, 20 data directories, 9 scheduled jobs.
  ⚠️ **The first version put a file count beside each data directory and that was a bug, not a
  detail.** The collectors write into those every day, so a committed map carrying a count would go
  stale on its own within the hour and its currency guard would fail hourly for nothing — the exact
  nuisance F1 had removed from the lint-debt guard two hours earlier. Removed, with the reason on
  the page: the directory list is stable, what is in them is not.
  `tests/unit/test_system_map_is_current.py` pins the committed map to a fresh render, checks the
  render is **deterministic** (two renders identical, or the guard would fail at random), floors
  the map's size and sections so a render over an empty repository could not satisfy equality, and
  asserts **every script states what it is** — 192 of 192 do. Marked `workspace_evidence` on
  purpose: the map includes this machine's launchd agents, which a CI runner legitimately does not
  have.
  The pre-commit hook from F1 now regenerates the map as well, on any commit touching Python,
