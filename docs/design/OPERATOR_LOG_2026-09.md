...
- 2026-09-15 23:10Z. PUBLISHER TREE PULLED TO 76ed48c; ONE HEALTH TEST IS EXPECTED TO FAIL TONIGHT.
  The publisher tree took the merged data-rights and cash-financing work at 23:10:19Z while the
  hourly tick was idle. `artifacts/publication/all_sleeve_data_rights_audit.json` was persisted
  before the rights policy gained Bybit, OKX, Gate and Kraken, so it now counts fewer sources than
  `config/data_source_rights_policy.json` holds, and tonight's 23:32Z health suite is expected to
  fail `test_persisted_all_sleeve_audit_matches_current_sources`. That artifact is regenerated by
  step 1b of the nightly publish chain, so the failure clears at the 22:10Z chain on 2026-09-16. It
  was deliberately not regenerated by hand: steps 1, 2, 5, 8 and 9 of the chain and the external
  submission plan bind its hash, and rewriting it outside the chain would break those bindings for
  the sake of one night's red test.
