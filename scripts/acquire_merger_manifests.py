"""Bounded acquisition of filing-index SEC pages for the immutable merger sample."""
...
async def main():
        ...
        json.dumps(
            {
                ...
                "scope": "395 filing indexes; preserve400 mappings; no bodies or returns",
                "concurrency": 2,
    ...
    async def acquire(row, client):
        ...
        async with semaphore:
            ...
            path = OUT / "manifests" / f"{row.accession}.html"
            record["url"] = (
                f"https://www.sec.gov/Archives/edgar/data/{row.cik}/"
                f"{row.accession.replace('-', '')}/{row.accession}-index.html"
            )
            try:
                if path.exists():
                    ...
                    async with client.stream("GET", record["url"]) as response:
                        ...
                        async for part in response.aiter_bytes():
                            ...
                            chunks.append(part)
                    data = b"".join(chunks)
                    if b"Document Format Files" not in data or row.accession.encode() not in data:
                        raise ValueError("Index/accession identity absent")
