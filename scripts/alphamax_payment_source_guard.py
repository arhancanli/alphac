...
def validate_source(review_dir, *, root, frozen_audit, frozen_sha256, start, end):
    ...
    for required in [review_dir/'reviewed_schedule.json', review_dir/'unresolved.json', frozen_audit]:
    ...
    def index(rows):
        ...
        return result
    baseline=index(original);accepted=index(reviewed);pending=index(unresolved)
    if set(accepted)&set(pending) or set(accepted)|set(pending)!=set(baseline):
        raise ValueError('Source partition drops or adds frozen events')
    if gate['total']!=len(baseline) or gate['reviewed']!=len(accepted) or gate['unresolved']!=len(pending):
    ...
    return rows, {'gate_sha256':sha(review_dir/'gate.json'),'frozen_audit_sha256':frozen_sha256,
                  'start':start,'end':end,'reviewed_events':len(rows),'point_in_time_proven':False}
