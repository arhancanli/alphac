...
Version 2 adds explicit exclusions; source evidence correctness still needs review.
"""
import hashlib
import json
...
def validate_source(review_dir, *, root, frozen_audit, frozen_sha256, start, end):
    ...
    def index(rows):
        ...
        return result
    baseline=index(original);accepted=index(reviewed);pending=index(unresolved);removed=index(excluded)
