"""Calendar-corrected exposure diagnostics; original misaligned calculation preserved."""
...
def align_factors(bars):
    """Actual close at bar t is marked at the next exchange-session open timestamp."""
    ...
    returns.index = pd.Index(
        [sleeve.calendar.next_bar_open(int(ts), sleeve.anchor_tf) for ts in returns.index],
...
def main():
    ...
    if (
        ...
        raise ValueError("Corrected analysis source drift")
    ...
    joined.to_parquet(OUT / "exposure_observations_corrected.parquet")
    with (OUT / "exposure_results_corrected.json").open("x") as f:
