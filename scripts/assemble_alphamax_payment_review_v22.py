...
R=Path(__file__).resolve().parents[1];S=R/'evidence/alphamax-payment-source-full-20260913';O=R/'evidence/alphamax-payment-schedule-review-v22-20260913'
...
assert len(accepted)==3399 and len(unresolved)==67 and len(review)==len(accepted)
...
files.extend([G,C,D,E,F,H,J,K,L,M,N,Q,T,V,W,Y,Z,KK,LL,PP,HH,DD])
for f in files:
...
report={'status':'INCOMPLETE_SOURCE_SCHEDULE_REPLAY_FORBIDDEN','total':3466,'reviewed':3399,'unchanged_amount':3292,'explicit_new_amounts':107,'unresolved':67,'unresolved_by_symbol':dict(Counter(x['symbol'] for x in unresolved)),'coverage_by_year':{str(y):{'reviewed':sum(x['ex_date'].startswith(str(y)) for x in accepted),'unresolved':sum(x['ex_date'].startswith(str(y)) for x in unresolved)} for y in range(2022,2027)},'qualification':False,'limitations':'Assembled vendor/issuer-reviewed current-vintage events, not complete security-identity/PIT certification. No input lake mutated or historical returns computed. Do not drop unresolved events or trade only reviewed names.','sha256':{str(p.relative_to(R)):sha(p) for p in [*files,Path(__file__),O/'reviewed_schedule.json',O/'unresolved.json']}}
