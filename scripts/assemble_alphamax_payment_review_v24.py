...
R=Path(__file__).resolve().parents[1];S=R/'evidence/alphamax-payment-source-full-20260913';O=R/'evidence/alphamax-payment-schedule-review-v24-20260913'
...
for x in json.loads(Z.read_text())['resolved']:
 assert key(x) not in review
 review[key(x)]={k:x[k] for k in ['amount','pay_date','basis','amount_changed','frozen_amount']}
KK=S/'KKR_adjudication.json'
for x in json.loads(KK.read_text())['resolved']:
 assert key(x) not in review
 review[key(x)]={k:x[k] for k in ['amount','pay_date','basis','amount_changed']}
 if x['amount_changed']:review[key(x)]['frozen_amount']=x['frozen_amount']
LL=S/'LIN_adjudication.json'
for x in json.loads(LL.read_text())['resolved']:
 assert key(x) not in review
 review[key(x)]={k:x[k] for k in ['amount','pay_date','basis','amount_changed']}
 if x['amount_changed']:review[key(x)]['frozen_amount']=x['frozen_amount']
PP=S/'PSA_adjudication.json'
for x in json.loads(PP.read_text())['resolved']:
 assert key(x) not in review
 review[key(x)]={k:x[k] for k in ['amount','pay_date','basis','amount_changed']}
HH=S/'HL_adjudication.json'
for x in json.loads(HH.read_text())['resolved']:
 assert key(x) not in review
 review[key(x)]={k:x[k] for k in ['amount','pay_date','basis','amount_changed','frozen_amount']}
DD=S/'domestic_five_adjudication.json'
for x in json.loads(DD.read_text())['resolved']:
 assert key(x) not in review
 review[key(x)]={k:x[k] for k in ['amount','pay_date','basis','amount_changed','frozen_amount']}
MM=S/'MS_RF_O_adjudication.json'
for x in json.loads(MM.read_text())['resolved']:
 assert key(x) not in review
 review[key(x)]={k:x[k] for k in ['amount','pay_date','basis','amount_changed','frozen_amount']}
GG=S/'GM_CF_BX_adjudication.json'
for x in json.loads(GG.read_text())['resolved']:
 assert key(x) not in review
 review[key(x)]={k:x[k] for k in ['amount','pay_date','basis','amount_changed']}
accepted=[];unresolved=[]
for x in source['matches']:
...
assert len(accepted)==3405 and len(unresolved)==61 and len(review)==len(accepted)
...
files.extend([G,C,D,E,F,H,J,K,L,M,N,Q,T,V,W,Y,Z,KK,LL,PP,HH,DD,MM,GG])
for f in files:
...
report={'status':'INCOMPLETE_SOURCE_SCHEDULE_REPLAY_FORBIDDEN','total':3466,'reviewed':3405,'unchanged_amount':3295,'explicit_new_amounts':110,'unresolved':61,'unresolved_by_symbol':dict(Counter(x['symbol'] for x in unresolved)),'coverage_by_year':{str(y):{'reviewed':sum(x['ex_date'].startswith(str(y)) for x in accepted),'unresolved':sum(x['ex_date'].startswith(str(y)) for x in unresolved)} for y in range(2022,2027)},'qualification':False,'limitations':'Assembled vendor/issuer-reviewed current-vintage events, not complete security-identity/PIT certification. No input lake mutated or historical returns computed. Do not drop unresolved events or trade only reviewed names.','sha256':{str(p.relative_to(R)):sha(p) for p in [*files,Path(__file__),O/'reviewed_schedule.json',O/'unresolved.json']}}
