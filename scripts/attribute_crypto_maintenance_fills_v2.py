...
SOURCE=ROOT/'artifacts/analysis/crypto_extended_reference_20260913'
OUT=ROOT/'evidence/crypto-maintenance-fill-attribution-20260913-v2'
HOUR=3600000
...
def main():
    ...
    for arm in ['baseline','baseline_stress']:
        ...
        for leg in wf['legs']:
            ...
            counters=leg['risk_counters'];assert counters['bars_halted_flat']==0 and counters['hold_stale_signal']==0 and counters['hold_cov_cold_start']==0 and counters['hold_degenerate_xsection']==0
            decisions=[];next_rebalance=None
            for row in eq.itertuples():
                previous=ladder.state
                state=ladder.update(float(row.equity));assert state is not DDState.FLAT_HALTED
    ...
    grouped=data.groupby(['arm','category']).agg(fills=('fee','size'),fees_quote=('fee','sum'),notional_quote=('notional','sum'),reducing_fills=('reduces_absolute_qty','sum'),below_band_fills=('below_nav_band_at_decision','sum')).reset_index();grouped.to_csv(OUT/'summary.csv',index=False)
