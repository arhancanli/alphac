"""Reconcile ten additional retained JPMorgan notices without admitting cash corrections."""
import re,json,hashlib
from pathlib import Path
from decimal import Decimal
from datetime import datetime
R=Path(__file__).resolve().parents[1];O=R/'evidence/alphamax-asml-jpm-gap-notices-20260913';U=R/'evidence/alphamax-payment-schedule-review-v14-20260913/unresolved.json'
...
assert len(rows)==10
report={'status':'TEN_ADDITIONAL_NOTICE_CASH_IDENTITIES_RECONCILED_NOT_REPLAY_READY','rows':rows,'vendor_gross_exact_count':sum(x['vendor_gross_exact'] for x in rows),'vendor_record_exact_count':sum(x['vendor_record_exact'] for x in rows),'limits':'EUR times FX and gross minus reported withholding/fees equal net exactly in each notice. Report printed tax rounding residual, do not replace reported amount. Joined by unique USD payment date; explicit record mismatches remain unresolved. USD ex-entitlement availability, withheld cash, holder tax/reclaims, and remaining notice coverage need policy/evidence. No correction applied, no historical performance.','sha256':{str(p.relative_to(R)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),U,*sorted(O.glob('*.bin')),*sorted(O.glob('*.txt')),*sorted(O.glob('*.receipt.json')),O/'request_plan.json',O/'public_dividends.json',O/'public_dividends.receipt.json']}}
