...
for symbol in protocol['symbols']:
 path=O/f'{symbol}_dividends.bin';receipt_path=O/f'{symbol}_dividends_receipt.json'
 if not receipt_path.exists():
  assert (O/f'{symbol}_dividends_failure.json').exists()
  body={'results':[]}
 else:
  receipt=json.loads(receipt_path.read_text());assert sha(path)==receipt['sha256'];bindings[str(path.relative_to(R))]=sha(path)
  if receipt['http_status']!=200:body={'results':[]}
  else:
   body=json.loads(path.read_bytes())
   if body.get('status')!='OK' or body.get('next_url'):body={'results':[]}
...
report={'status':'FULL_SOURCE_MATCH_AUDITED','expected':len(rows),'accepted':sum(x['accepted'] for x in rows),'rejected':sum(not x['accepted'] for x in rows),'matches':rows,'qualification':False,'limits':'Current-vintage reference; ticker/date/amount match is not historical security identity or publication timing certification. No new strategy returns computed.','sha256':{**bindings,str(Path(__file__).relative_to(R)):sha(Path(__file__)),str((O/'protocol.json').relative_to(R)):sha(O/'protocol.json'),str((O/'expected_actions.json').relative_to(R)):sha(O/'expected_actions.json')}}
