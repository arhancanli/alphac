...
def run(arm):
 ...
 baseline=read(ROOT/'artifacts/analysis/combined_2022_eligible_core_20260913'/('baseline' if arm=='normal' else 'baseline_stress')/'daily.csv')
 ...
 assert len(ledger)==len(combined)==len(baseline)==365
 ...
 gate={'excess_gain':result['excess_sharpe_proxy']-result['baseline']['excess_sharpe_proxy']>=0,
       ...
       'annual_excess_floor':all(v['candidate']['excess_sharpe_proxy']-v['baseline']['excess_sharpe_proxy']>=0 for v in result['annual'].values())}
