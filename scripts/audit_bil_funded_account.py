...
def run(arm):
 ...
 baseline=read(ROOT/'artifacts/analysis/crypto_continuous_account_20260913/baseline/calendar2023_2026.csv')
 ...
 qty=0;spent=D(0);entitlements=[];mark=D(0);previous=D(32500);month=None;max_error=D(0)
 ...
 for row,output,base in zip(ledger,combined,baseline):
  ...
  available=D(32500)-spent+paid_before
  ...
  cash=D(32500)-spent+paid_to_date
  nav=D(32500)+qty*mark-spent+sum((amount for _,amount in entitlements),D(0))
 ...
 report={'pass':True,'arm':arm,'days':len(ledger),'source_bindings_verified':len(manifest),'max_nav_error':str(max_error),
         ...
         'method':'Independent cumulative purchase/debit and issuer-entitlement reconstruction; standalone32500 capital, benchmark and returns checked.',
