...
def main():
    ...
    for key,name in [('max','alphamax_session_cooldown_20260913'),('trend','trend_extended_reference_20260913')]:
        p=ROOT/'artifacts/analysis'/name/(arm if key=='max' else arm.replace('candidate','baseline'))/'run/equity.parquet';refs[str(p.relative_to(ROOT))]=sha(p)
    ...
    p=ROOT/'artifacts/analysis/crypto_risk_normalized_rank_20260913'/arm/'run/equity.parquet';refs[str(p.relative_to(ROOT))]=sha(p)
