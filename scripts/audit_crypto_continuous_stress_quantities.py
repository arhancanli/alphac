...
import json,hashlib
import pandas as pd
R=Path(__file__).resolve().parents[1];O=R/'artifacts/analysis/crypto_continuous_account_20260913/baseline_stress'
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
wf=json.loads((O/'run/walkforward.json').read_text());initial=22500.;legs=[]
assert len(wf['legs'])==1, 'One physical ledger required'
for leg in wf['legs']:
...
report={'status':'CONTINUOUS_ACCOUNT_QUANTITIES_AND_NAV_RECONCILED','initial_cash':22500,'final_nav':initial,'funded_continuity_pass':True,'material_flat_restart_boundaries':sum(x['boundary_converted_to_flat_next_leg'] for x in legs),'legs':legs,'limits':'One physical account from inception through horizon; final open positions remain marked assets, not cash. Recorded execution prices/fees and mark prices are inputs. This independently checks quantities, cost basis, realized/unrealized PnL, funding position quantities and account conservation; it does not certify market impact, quotes, actual collateral or price-source availability.','sha256':{str(p.relative_to(R)):sha(p) for p in [Path(__file__),O/'run/walkforward.json']+sorted((O/'run').rglob('*.parquet'))}}
