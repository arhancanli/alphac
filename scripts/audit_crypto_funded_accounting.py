...
def main():
    ...
    curves = []
    initial = 22500.0
    assert meta["legs"][0]["test_start"] == 1672531200000
    assert meta["legs"][0]["summary"]["initial_equity"] == initial
    for leg in meta["legs"]:
        ...
        assert fills.empty or fills.ts.min() >= 1672531200000
        for extra in ["financing.parquet", "corporate_actions.parquet"]:
            assert pd.read_parquet(directory/extra).empty
    ...
    report = {
        "status": "FUNDED_CRYPTO_SAVED_ACCOUNTING_RECONCILED",
        "initial_cash": 22500,
        "inception": 1672531200000,
        "limits": "Saved realized/unrealized fields reconciled, not full independent fill-quantity reconstruction. No pre-inception fills; funding and fees included once. Formal calendar benchmark audit separate.",
        "legs": residuals,
