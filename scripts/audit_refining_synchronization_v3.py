...
def grid_table(rows):
    """Preserve nullable nanoseconds and fields absent from the first record."""
    schema = pa.schema(
        [
            ("date", pa.string()),
            ("decision_ns", pa.int64()),
            ("valid", pa.bool_()),
            ("reason", pa.string()),
            *[
                (name, pa.int64())
                for name in (
                    ...
                    "displayed_two_way_recipes",
                    "max_age_ns",
                    "cross_leg_skew_ns",
                )
            ],
            *[
                (root + suffix, pa.int64())
                for root in ROOTS
                for suffix in ("_recv_ns", "_event_ns")
            ],
        ]
    )
    if any(set(row) - set(schema.names) for row in rows):
        raise ValueError("Unrecognized audit fields")
    table = pa.Table.from_pylist(rows, schema=schema)
    expected = [{name: row.get(name) for name in schema.names} for row in rows]
    if table.to_pylist() != expected:
        raise ValueError("Audit serialization changed values")
    return table
...
def main():
    ...
    table = grid_table(all_rows)
    pq.write_table(table, OUT / "grid_audit.parquet")
    if not pq.read_table(OUT / "grid_audit.parquet").equals(table):
        raise ValueError("Parquet round trip changed audit")
