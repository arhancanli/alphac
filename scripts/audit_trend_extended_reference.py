...
def main():
    ...
    packet_path=ROOT/'artifacts/research/trial_packets'/f"{r['hypothesis_identity']}.json";packet=json.loads(packet_path.read_text());digest=packet.pop('content_hash');assert digest=='sha256:'+hashlib.sha256(json.dumps(packet,sort_keys=True,separators=(',',':')).encode()).hexdigest()
    ...
    with (d/'audit_closure.json').open('x') as stream:json.dump({'status':'AUDITED_RESEARCH_PACKET_NO_QUALIFICATION','packet':str(packet_path.relative_to(ROOT)),'packet_sha256':sha(packet_path),'audit_sha256':sha(d/'independent_audit.json')},stream,indent=2);stream.write('\n')
