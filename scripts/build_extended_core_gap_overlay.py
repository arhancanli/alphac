...
SOURCE = ROOT / "evidence/core-2023-2026-gap-archives-20260913"
OUT = ROOT / "evidence/core-2023-2026-gap-overlay-20260913"
...
def main():
    ...
    manifest = json.loads(
        (ROOT / "evidence/core-2023-2026-coverage-20260913/source_manifest.json").read_text()
    )
    ...
    for iid, items in patches.items():
        source = ROOT / "artifacts/analysis/crypto_full2022_terminal_20260913/state/repaired_snapshot/ohlcv" / f"instrument_id={iid}" / "year=2022/data.parquet"
        expected = manifest["sha256"][str(source)]
    ...
    assert sum(r["added"] for r in results) == 312
        ...
        json.dumps(
            {
                ...
                "limitations": "Three patched yearly partitions only; "
