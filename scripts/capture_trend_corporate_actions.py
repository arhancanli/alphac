...
OUT = ROOT / "evidence/alphatrend-action-capture-20260912"
...
async def main():
    ...
    write(
        "protocol.json",
        {
            ...
            "bindings": {
                str(p): hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__), prior]
            },
        },
    )
    ...
    async with httpx.AsyncClient(timeout=12, follow_redirects=False, trust_env=False) as client:
        ...
        results = await asyncio.gather(
            *(get(s, k) for s in symbols for k in ["splits", "dividends"])
        )
    write(
        "result.json",
