...
def main():
    ...
    parser.add_argument("arm", choices=["candidate", "candidate_stress"])
    ...
    feature = json.loads((directory / "feature_audit.json").read_text())
    assert feature["maximum_scalar_difference"] < 1e-12
    for path, digest in feature["sha256"].items():
        assert sha(ROOT/path) == digest
    policy = {
        "benchmark": "Current-vintage DFF proxy; strict prior publication-date match; ACT/360",
        "session_interval": "Compound daily simple proxy accruals on (previous source session, current source session]; Dec30 2022 predecessor; final June1 2026 session.",
        "annualization": 252,
        "qualification": False,
    ...
    frame = pd.read_csv(directory / "calendar2023_2026.csv")
    ...
    assert len(sessions) == 855
    days = pd.DataFrame({"day": pd.date_range("2022-12-31", sessions.iloc[-1], freq="D")})
    rates = (
        pd.read_parquet(DFF)
    ...
    previous = pd.Timestamp("2022-12-30")
    benchmark = []
    counts = []
    for day in sessions:
        ...
        previous = day
    assert sum(counts) == len(days) == (pd.Timestamp("2026-06-01") - pd.Timestamp("2022-12-30")).days
    ...
    frame.to_csv(directory / "calendar2023_2026_excess.csv", index=False)
    ...
    limitation = (
        "Retrospective standalone dividend-aware momentum candidate, not untouched OOS or combined qualification. "
        ...
        "Historical action publication and completeness are not independently certified; ex-date reinvestment is signal-only. "
        "The last source session is June1 2026; unresolved old EWMA split boundaries remain. "
        "Feature values reconstructed from bound source/code; runtime feature snapshot absent. "
        "Ledger reconciliation uses saved realized/unrealized fields and independently recomputed borrow; "
        "full raw-fill quantity/impact reconstruction is not claimed."
    )
    report = {
        ...
        "calendar_accrual_days": len(days),
        "session_observations": 855,
        ...
        "source_sha256": {
            ...
            for p in [
                DFF,
                directory / "calendar2023_2026.csv",
                directory / "calendar2023_2026_excess.csv",
    ...
    with (directory / "FINAL_REPORT.md").open("x") as stream:
        stream.write(
            ...
            f"2023–June2026 session return {audit['total_return']:.6%}; raw Sharpe {audit['raw_sharpe']:.6f}; "
            f"DFF excess Sharpe proxy {excess_sharpe:.6f}; maxdrawdown {audit['max_drawdown']:.6%}. "
            "855 session returns with observed Dec30 2022 predecessor; all857 engine marks reconcile. "
            "Fourteen legs: thirteen63-session legs and one38-session leg; durable outputs match final parquet files.\n\n"
            + limitation
            + "\n"
        )
    ...
    files += [
        ...
        Path(__file__),
        ROOT / "scripts/audit_alphamax_total_return_accounting.py",
    ]
    ...
    write(
        ...
        {
            "status": "ALPHAMAX_TOTAL_RETURN_CANDIDATE_ACCOUNTED_PACKET_CLOSED",
            "qualified": False,
