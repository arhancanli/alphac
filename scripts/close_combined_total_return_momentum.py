"""Close the measured failed dividend-aware momentum combined identity without admission."""
from pathlib import Path
import json,hashlib
R=Path(__file__).resolve().parents[1];D=R/'artifacts/analysis/combined_total_return_momentum_20260913/normal'
...
limitation='Retrospective fixed dividend-aware momentum substitution in an existing AlphaMax sleeve. Normal combined excess gain and no-lower-return gates failed; stop candidate, no stress/2022 or parameter rescue. No admission, new distinct sleeve, live qualification or publication claim. Current-vintage sources, modeled metadata/borrow/action timing and daily fixed subbook-weight proxy remain. Standalone feature reconstruction is from bound source/code; runtime feature snapshot absent; full raw-fill quantity/impact reconstruction not claimed.'
paths=[p for p in D.iterdir() if p.is_file() and p.name!='closure.json']+[Path(__file__),R/'evidence/alphamax-dividend-momentum-feasibility-20260913/DESIGN.json',R/'artifacts/analysis/alphamax_total_return_momentum_20260913/candidate/closure.json']
...
write(D/'closure.json',{'status':'TOTAL_RETURN_NORMAL_COMBINED_FAILED_CLOSED','qualified':False,'packet':str(p.relative_to(R)),'packet_sha256':sha(p),'audit_sha256':sha(D/'independent_audit.json'),'stress_allowed':False,'2022_allowed':False})
