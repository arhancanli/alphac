...
class ContinuousDepositaryEquityRunner(WalkForwardRunner):
    def _run_leg_set(self,splitter,grid,ids,full_frame,full_ts,*,train_bars,allocator,band,
        ...
        if not isinstance(engine,DepositaryEquityBacktester):
...
def continuous_depositary_factory(directory,*,payments,depositary_terms,retrospective_vintage_ms,retrospective_actions):
    """Bind complete action/payment schedule and save once after model transitions."""
    from pathlib import Path
    from alphaforge.backtest.depositary_engine import DepositaryEquityBacktester
    directory=Path(directory)
    class DurablePayable(DepositaryEquityBacktester):
        def __init__(self,*args,**kwargs):
            self.destination=directory/'000'
            self.destination.mkdir(parents=True,exist_ok=False)
            super().__init__(*args,payments=payments,depositary_terms=depositary_terms,retrospective_vintage_ms=retrospective_vintage_ms,
