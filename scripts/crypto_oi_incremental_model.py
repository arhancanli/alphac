...
CONTROLS = ('funding_mean_21d', 'momentum_7d', 'momentum_28d', 'eth_indicator')
EXTRA = 'oi_crowding'
...
def fit_pair(frame, cutoff):
    ...
    f = f[(f.decision_at >= pd.Timestamp('2022-01-01', tz='UTC')) & (f.decision_at < cutoff) & (f.label_available_at < cutoff-pd.Timedelta(days=7))].copy()
    if (f.input_available_at > f.decision_at).any() or (f.label_available_at < f.decision_at+pd.Timedelta(days=7,hours=1)).any():
        raise ValueError('Noncausal input or label')
    if (f.decision_at != f.decision_at.dt.normalize()).any():
        raise ValueError('Decision must be UTC midnight')
    ...
    if len(groups)<252 or not all(g==SYMBOLS for g in groups):
        raise ValueError('Need >=252 paired dates')
