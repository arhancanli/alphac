...
#
# RE-DERIVED AGAIN 2026-08-20, same procedure — and this time the cause is a CORRECTION we
# published against ourselves, not a book we chose to change. On 2026-08-16 04:09 the
# AlphaVintage leg's curve (artifacts/probe/cpi_surprise_size/equity.parquet, the file
# load_probe_curve reads) was regenerated by the calendar-corrected re-run that retains
# zero-exposure sessions between first and last activity. That re-run took the sleeve's own
# net Sharpe 0.3382 -> 0.2298 and its Newey-West t to 1.267, failing the b_nw_t_ge_1p5 check
# the candidate itself pre-registered; its artifact records verdict KILLED. The sleeve holds
# a quarter of BOOK_WEIGHTS, so the book moved with it the same day: 1.82 -> 1.78 and
# 1.42 -> 1.38, first observed by this monitor on the 2026-08-16 run and identical on every
# run since (08-16, 08-18, 08-19, 08-20) — a step with a cause, not a drift.
# ATTRIBUTION CHECKED, NOT ASSUMED: the other three legs feed from curves unchanged since
# June (k30_dn_63, crypto_carry_wk, managed_futures), and portfolio/book.py has not been
# touched since 2026-08-04, so the whole move belongs to the one corrected leg.
# Direction is DOWN on both. A correction that costs us is the only direction that earns a
