...
CONTROL=ROOT/'artifacts/analysis/combined_session_cooldown_20260913'
...
def main():
    ...
    for arm,base in [('candidate','candidate'),('candidate_stress','candidate_stress')]:
        ...
        for col in ['max','trend','benchmark','cash_contribution']:np.testing.assert_array_equal(x[col],y[col])
        np.testing.assert_allclose(x.total-y.total,.225*(x['crypto']-y['crypto']),atol=1e-15,rtol=0)
    ...
    turnover=pd.read_csv(ROOT/'artifacts/analysis/crypto_rank_retention_20260913/comparison.csv')
    for r in turnover.itertuples():checks[r.arm+'_scheduled_turnover_reduction']=r.scheduled_turnover_reduction>=.1
    refs['artifacts/analysis/crypto_rank_retention_20260913/comparison.csv']=sha(ROOT/'artifacts/analysis/crypto_rank_retention_20260913/comparison.csv')
    passed=all(checks.values());decision='PASS_BROADER_GATES_REQUIRE_SEPARATE_2022_CHECK' if passed else 'FAIL_STOP_RETENTION_FAMILY_NO_WIDTH_SWEEP'
    ...
    report='# Combined one-rank crypto retention comparison\n\nFrozenJan2023–June1 2026; only crypto retention changes. AlphaMax,confirmedTrend,benchmark and cash contributions exactly unchanged on every date.\n\n| Metric | Baseline | Candidate | Baseline stress | Candidate stress |\n|---|---:|---:|---:|---:|\n'
    ...
    report+='Both combined paths independently reconstructed from saved engine curves and DFF; all2,496daily observations reconcile. Daily weights22.5% each+32.5%zero-yieldcash, source-session equity/fullUTCcrypto conventions unchanged. Saved-path difference equals.225 times the crypto difference, confirming no hidden component substitution.\n\n'
    report+=('Next: separately preregister2022 normal/stress validation of the same one-rank retention rule; no promotion yet. ' if passed else 'Stop this retention family as preregistered. Preserve candidate and baseline; do not sweep nearby widths or relax annual gates. Pivot to independent alpha research. ')
