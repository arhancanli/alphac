...
OUT=ROOT/'artifacts/analysis/combined_crypto_risk_rank_20260913'
CONTROL=ROOT/'artifacts/analysis/combined_session_cooldown_20260913'
SPEC=ROOT/'evidence/combined-crypto-risk-rank-20260913/EXPERIMENT_SPEC.json'
...
def main():
    ...
    passed=all(checks.values());decision='PASS_BROADER_GATES_REQUIRE_SEPARATE_2022_CHECK' if passed else 'FAIL_STOP_RISK_NORMALIZED_RANK_FAMILY_NO_SWEEP'
    ...
    report='# Combined risk-normalized crypto ranking comparison\n\nFrozenJan2023–June1 2026; only crypto asset selection changes. AlphaMax,confirmedTrend,benchmark and cash contributions exactly unchanged on every date.\n\n| Metric | Baseline | Candidate | Baseline stress | Candidate stress |\n|---|---:|---:|---:|---:|\n'
    ...
    report+=('Next: separately preregister2022 normal/stress validation of the same risk-normalized ranking rule; no promotion yet. ' if passed else 'Stop this risk-normalized ranking family as preregistered. Preserve candidate and baseline; do not sweep nearby ranking exponents or covariance windows or relax annual gates. Pivot to independent alpha research. ')
