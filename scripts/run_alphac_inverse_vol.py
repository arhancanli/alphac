"""Execute exactly the sealed inverse-vol allocation experiment and mandatory cost stress."""
...
from alphaforge.portfolio.research_inverse_vol import allocate
...
OUT = ROOT / "artifacts/analysis/alphac_inverse_vol_20260913"
...
def main():
    ...
    spec_path = ROOT / "evidence/alphac-inverse-vol-20260913/EXPERIMENT_SPEC.json"
    ...
    assert len(control) == 1061 and all(
        frame.index.to_series().diff().iloc[1:] == pd.Timedelta(days=1)
    )
    ...
    for arm, cost_rate in [("primary", 0.0001), ("cost_stress", 0.0005)]:
        ...
        config = {
            "family": "alphac_combined_inverse_vol_allocation",
            "candidate": spec["candidate"],
            "input_sha256": spec["control_component_csv_sha256"],
            "cost_rate": cost_rate,
            "spec_sha256": sha(spec_path),
            "implementation_sha256": sha(ROOT / "src/alphaforge/portfolio/research_inverse_vol.py"),
        }
        prereg = {
        ...
        reservation.update(
            ...
            trial_config=config,
            family_trial_account="alphac_combined_inverse_vol_allocation",
            return_identity_id=f"alphac_inverse_vol_{arm}_20260913",
            hypothesis_identity=hypothesis_hash(config),
            packet_public_path=f"/glassbox/trial-packets/alphac-inverse-vol-{arm}-20260913",
            paper_public_path="/research/alphac-inverse-vol-20260913",
        )
        ...
        net, weights, cost = allocate(frame.to_numpy(), weekdays, cost_rate)
        result = {
            "metrics": metrics(net),
            "control": baseline,
            ...
            "worst_control_day_weights": weights[np.argmin(control)].tolist(),
            "yearly": {
                str(year): {
        ...
        pd.DataFrame(
            {
                ...
                "added_cost": cost,
                "candidate": net,
                **{f"weight_{name}": weights[:, i] for i, name in enumerate(frame.columns[:4])},
            },
            index=frame.index,
