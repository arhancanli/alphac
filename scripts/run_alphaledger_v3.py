#!/usr/bin/env python3
"""AlphaLedger v2 — execute PREREG_ALPHALEDGER_V3.md: eq_asset_growth with a PIT liquidity floor.
...
looser per-rebalance version would need engine support and is not silently substituted here.
    ...
    uv run python scripts/run_alphaledger_v3.py
"""
# ruff: noqa: E501
...
PREREG = _ROOT / "docs/design/PREREG_ALPHALEDGER_V3.md"
ALLOWLIST = _ROOT / "data/research/universe_allowlist_20260619.json"
V1_ARTIFACT = _ROOT / "artifacts/walkforward/prereg_investment"
OUT_DIR = _ROOT / "artifacts/walkforward/alphaledger_v3"
LEDGER = _ROOT / "var/experiments.jsonl"
MIN_ADV = 25_000.0


def _eligible(ids: list[str], lake: Path) -> list[str]:
    """Names whose ROLLING 30-session median dollar volume never falls below MIN_ADV.

    THE STATISTIC MATTERS AND v2 GOT IT WRONG. The engine caps each order at 1% of the decision
    bar's rolling 30-session median dollar volume and HALTS when that floors below one lot. v2
    filtered the static full-history median, which cannot prevent a transient thin window: a name
    with a $10M lifetime median can sit at a few hundred dollars for a month. Same window length,
    same median, or the filter does not bind on what the engine checks.
    """
    keep, thin, nodata = [], 0, 0
    for iid in ids:
        ...
        try:
            t = ds.dataset(p, format="parquet").to_table(
                columns=["ts_open", "close", "volume"]).to_pandas()
        except Exception:
            nodata += 1
            continue
        if len(t) < 60:
            nodata += 1
            continue
        t = t.sort_values("ts_open")
        dv = t["close"].to_numpy(dtype="float64") * t["volume"].to_numpy(dtype="float64")
        roll = pd.Series(dv).rolling(30, min_periods=30).median().dropna()
        if len(roll) and float(roll.min()) >= MIN_ADV:
            keep.append(iid)
        else:
            thin += 1
    print(f"  rolling-30d floor ${MIN_ADV:,.0f}: kept {len(keep)}, dropped {thin} ever-thin, "
...
def main() -> int:
    ...
    out = _ROOT / "artifacts/analysis/alphaledger_v3"
