...
DESIGN = ROOT / "evidence/alphamax-covariance-basis-20260913/EXPERIMENT_SPEC.json"
...
def prepare():
    previous = ROOT / "artifacts/analysis/alphamax_share_ratio_20260913"
    ...
    for p in [
        ...
        ROOT / "scripts/alphamax_covariance_engine.py",
        ROOT / "scripts/alphamax_session_price_basis.py",
        ROOT / "tests/unit/test_alphamax_covariance_seam.py",
        ROOT / "tests/unit/test_alphamax_remaining_price_basis.py",
        previous / "momentum_sigma/run/equity.parquet",
        previous / "momentum_sigma_stress/run/equity.parquet",
        previous / "momentum_sigma/closure.json",
        previous / "momentum_sigma_stress/closure.json",
    ]:
...
def execute(arm_id):
    ...
    reservation.update(
        ...
        family_trial_account="alphamax_covariance_basis",
        return_identity_id=f"alphamax_covariance_basis_{arm_id}_20260913",
        packet_public_path=f"/glassbox/trial-packets/alphamax-covariance-basis-{arm_id}-20260913",
        paper_public_path=f"/research/alphamax-covariance-basis-{arm_id}-20260913",
    )
