...
OUT = ROOT / "artifacts/analysis/alphamax_full2022_baseline_v2_20260913"
DESIGN = ROOT / "evidence/alphamax-full2022-baseline-v2-20260913/EXPERIMENT_SPEC.json"
...
def prepare():
    previous = ROOT / "artifacts/analysis/alphamax_full2022_baseline_20260913"
    ...
    inputs = {str(p.relative_to(ROOT)): sha(p)
              for p in sorted((OUT / "state").rglob("*")) if p.is_file()}
    for path, expected in original["sha256"].items():
        if not str(ROOT / path).startswith(str(previous / "state")):
            inputs[path] = expected
    for p in [DESIGN, Path(__file__), ROOT / "scripts/alphamax_replay_support_v2.py",
              previous / "input_manifest.json", previous / "baseline/closure.json",
              ROOT / "tests/unit/test_alphamax_replay_support_v2.py"]:
        inputs[str(p.relative_to(ROOT))] = sha(p)
    design = json.loads(DESIGN.read_text())
    write(OUT / "protocol.json", design)
    manifest = dict(original)
    manifest.update(sha256=inputs, protocol_sha256=sha(OUT / "protocol.json"),
...
def execute(arm_id):
    ...
    reservation.update(
        ...
        family_trial_account="alphamax_full2022_baseline_v2",
        return_identity_id=f"alphamax_full2022_v2_{arm_id}_20260913",
        packet_public_path=f"/glassbox/trial-packets/alphamax-full2022-v2-{arm_id}-20260913",
        paper_public_path=f"/research/alphamax-full2022-v2-{arm_id}-20260913",
    )
