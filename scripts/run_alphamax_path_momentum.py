...
DESIGN = ROOT / "evidence/alphamax-path-integration-20260913/EXPERIMENT_SPEC.json"
...
def prepare():
    ...
    files += [ROOT / "scripts" / p for p in ["alphamax_replay_support_v2.py", "alphamax_share_ratio_service.py", "alphamax_share_ratio_features.py", "alphamax_covariance_engine.py", "alphamax_session_price_basis.py", "alphamax_path_momentum.py", "alphamax_path_service.py"]]
...
def execute(arm_id):
    ...
    from alphamax_path_service import PathMomentumSignalService
    ...
    reservation.update(
        ...
        family_trial_account="alphamax_path_momentum",
        return_identity_id=f"alphamax_path_momentum_{arm_id}_20260913",
        packet_public_path=f"/glassbox/trial-packets/alphamax-path-momentum-{arm_id}-20260913",
        paper_public_path=f"/research/alphamax-path-momentum-{arm_id}-20260913",
    )
    ...
    with InstrumentStore(OUT / f"state/ops_cost{multiplier}.sqlite") as store:
        ...
        service = PathMomentumSignalService(
