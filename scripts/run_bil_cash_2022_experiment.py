...
OUT=ROOT/'artifacts/analysis/bil_cash_2022_20260913'
DESIGN=ROOT/'evidence/bil-cash-2022-experiment-20260913'
SPEC=DESIGN/'EXPERIMENT_SPEC.json'
BASE=ROOT/'artifacts/analysis/combined_2022_eligible_core_20260913'
...
def prepare():
    ...
    paths=[SPEC,DESIGN/'design_inputs.json',Path(__file__),ROOT/'scripts/bil_cash_ledger.py',ROOT/'tests/unit/test_bil_cash_ledger.py',ROOT/'pyproject.toml',ROOT/'uv.lock',BASE/'baseline/reservation.json']
...
def execute(arm):
    ...
    reservation=json.loads((BASE/'baseline/reservation.json').read_text())
    reservation.update(reserved_at=now.isoformat(),trial_config=config,hypothesis_identity=hypothesis_hash(config),
        family_trial_account=spec['family'],return_identity_id=f'bil_cash_2022_{arm}_20260913',
        packet_public_path=f'/glassbox/trial-packets/bil-cash-2022-{arm}-20260913',paper_public_path='/research/bil-cash-2022-20260913')
    ...
    try:
        ...
        baseline=readcsv(BASE/('baseline_stress' if arm=='stress' else 'baseline')/'daily.csv')
