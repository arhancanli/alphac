...
OUT=ROOT/'artifacts/analysis/bil_funded_account_20260913'
DESIGN=ROOT/'evidence/bil-funded-account-20260913'
...
LIMIT='Standalone32500 funded cash account. Current-vintage sources; assumed execution costs and end-pay-date settlement. Not untouched OOS, new alpha sleeve, qualification or launch evidence.'
...
def prepare():
    ...
    paths=[SPEC,DESIGN/'design_inputs.json',Path(__file__),ROOT/'scripts/bil_cash_ledger.py',ROOT/'evidence/combined-capital-transfer-audit-20260913/FUNDED_REPLAY_DESIGN.json',ROOT/'artifacts/analysis/crypto_continuous_account_20260913/baseline/calendar2023_2026.csv',ROOT/'tests/unit/test_bil_cash_ledger.py',ROOT/'pyproject.toml',ROOT/'uv.lock',BASE/'candidate/reservation.json']
...
def execute(arm):
    ...
    try:
        ...
        baseline=readcsv(ROOT/'artifacts/analysis/crypto_continuous_account_20260913/baseline/calendar2023_2026.csv')
        assert len(baseline)==len(ledger)==spec['evaluation']['calendar_days']
        assert [x['day'] for x in baseline]==[str(x['day']) for x in ledger]
        combined=[{'day':str(x['day']),'total':float(x['net_return']),'benchmark':float(b['benchmark']),'excess':float(x['net_return'])-float(b['benchmark'])} for b,x in zip(baseline,ledger)]
        ...
        result=stats(combined);result.update(qualified=False,limitations=LIMIT,initial_cash=32500)
        result['annual']={str(y):stats([r for r in combined if r['day'].startswith(str(y))]) for y in spec['evaluation']['annual_slices']}
