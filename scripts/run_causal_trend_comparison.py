"""Two fixed preregistered comparisons after causal and continuous-path parity gates."""
...
def prepare():
    ...
    write(
        OUT / "protocol.json",
        {
            ...
            "configuration": "17 ETFs, horizons 63/126/252, 10-session cadence, base costs",
...
def reserve(arm, ordinal, config, directory):
    ...
    prereg = {
        ...
        "scope": "Fixed causal-label continuous-book comparison; inspected history",
    ...
    reservation.update(
        ...
        return_identity_id=f"alphatrend_causal_continuous_{arm}_20260912",
        packet_public_path=f"/glassbox/trial-packets/alphatrend-causal-continuous-{arm}-20260912",
        paper_public_path=f"/research/alphatrend-causal-continuous-{arm}-20260912",
    )
    paths = {
...
def close_packet(directory, reservation):
    ...
    statements = {
        ...
        "execution_and_cost_model": "Single NextOpenFill backtest at original modeled costs. "
...
def execute():
    ...
    results = {}
    for arm, ordinal in [("baseline", 235), ("candidate", 236)]:
        directory = OUT / arm
        config = {
            k: v
        ...
        # Account immediately after returns exist, before rendering/report operations.
        ...
        (directory / "REPORT.md").write_text(
            f"# {arm}: causal continuous comparison\n\nAlready inspected history. No admission.\n\n"
