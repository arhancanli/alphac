...
SPEC = ROOT / "evidence/combined-2022-corrected-max-20260913/EXPERIMENT_SPEC.json"
...
def inputs():
    ...
    for arm in ARMS:
        paths.extend(ROOT / "artifacts/analysis/combined_2022_diagnostic_20260913" / arm / n for n in ["daily.csv", "result.json", "closure.json"])
...
def execute(arm):
    ...
    config = {
        "family": "combined_2022_corrected_max",
        "arm": arm,
    ...
    reservation = copy.deepcopy(json.loads((MAX / "sessions_splits/reservation.json").read_text()))
    reservation.update(
        reserved_at=now.isoformat(),
        trial_config=config,
        hypothesis_identity=hypothesis_hash(config),
        family_trial_account="combined_2022_corrected_max",
        return_identity_id=f"combined2022_corrected_max_{arm}_20260913",
        packet_public_path=f"/glassbox/trial-packets/combined2022-corrected-max-{arm}",
        paper_public_path="/research/combined2022-corrected-max-20260913",
    )
    ...
    max_arm = "sessions_splits_stress" if stress else "sessions_splits"
