...
SPEC = ROOT / "evidence/combined-2022-eligible-core-20260913/EXPERIMENT_SPEC.json"
TREND = ROOT / "artifacts/analysis/alphatrend_positive_targets_20260913"
MAX = ROOT / "artifacts/analysis/alphamax_covariance_basis_20260913"
...
ARMS = ["baseline", "baseline_stress"]
CONTROL = ROOT / "artifacts/analysis/combined_2022_corrected_max_20260913"
LIMIT = (
    "Retrospective eligible-core diagnostic selected after observing2022; not untouched validation. "
    "22.5% each Max/crypto/confirmedTrend; failedVintage22.5% and price-onlyoverlay10% become "
    "32.5% idlecash at zero yield. No reallocation to surviving sleeves. DFF benchmark charged "
    "against entire NAV; cash is not a new sleeve. Source-day marks, static subbook weights, "
    "financing/collateral, inter-sleeve rebalancing and PIT limitations remain. "
    "Normal/stressed subbook paths reused; no actual liquidation or live exposure change. "
    "Not qualified, no new independent alpha or launch evidence."
)
...
def inputs():
    paths = [SPEC, Path(__file__), ROOT / "pyproject.toml", ROOT / "uv.lock"]
    for arm in ARMS:
        paths.extend(CONTROL / arm / name for name in ["daily.csv", "result.json", "closure.json", "reservation.json", "independent_audit.json"])
...
def execute(arm):
    ...
    config = {
        "family": "combined_2022_eligible_core",
        "arm": arm,
    ...
    reservation.update(
        ...
        family_trial_account="combined_2022_eligible_core",
        return_identity_id=f"combined2022_eligible_core_{arm}_20260913",
        packet_public_path=f"/glassbox/trial-packets/combined2022-eligible-core-{arm}",
        paper_public_path="/research/combined2022-eligible-core-20260913",
    )
    ...
    frame = pd.read_csv(CONTROL / arm / "daily.csv", parse_dates=["day"])
    assert frame.day.tolist() == pd.date_range("2022-01-01", "2022-12-31").tolist()
