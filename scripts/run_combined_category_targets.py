...
OUT = ROOT / "artifacts/analysis/alphac_category_targets_20260913"
SPEC = ROOT / "evidence/alphac-category-targets-20260913/EXPERIMENT_SPEC.json"
SOURCE = ROOT / "artifacts/analysis/alphatrend_category_targets_20260913"
...
def main():
    ...
    assert ExperimentUnion.discover(OUT / "baseline/experiments.jsonl", ROOT).n_hypotheses() == 269
    ...
    for arm in ARMS:
        ...
        config = {
            "family": "alphac_category_targets",
            "arm": arm,
            "spec_sha256": sha(SPEC),
        ...
        reservation.update(
            ...
            family_trial_account="alphac_category_targets",
            return_identity_id=f"alphac_category_targets_{arm}_20260913",
            packet_public_path=f"/glassbox/trial-packets/alphac-category-targets-{arm}",
            paper_public_path="/research/alphac-category-targets-20260913",
        )
        ...
        evidence = {
            ...
            "runner": ROOT / "scripts/run_combined_category_targets.py",
        ...
        returns = pd.Series(
            x.equity.pct_change(fill_method=None).to_numpy(),
            index=pd.to_datetime(x.ts.to_numpy(), unit="ms"),
        )
        assert returns.index.is_unique
        assert returns.index.tz == frame.index.tz
        assert frame.index.min() > returns.index.min() and frame.index.max() <= returns.index.max()
        aligned = returns.reindex(frame.index)
        session_days = set(pd.to_datetime(expected, unit="ms").floor("D"))
