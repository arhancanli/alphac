...
SPEC = ROOT / "evidence/combined-extended-reference-20260913/EXPERIMENT_SPEC.json"
TREND = ROOT / "artifacts/analysis/trend_extended_reference_20260913"
MAX = ROOT / "artifacts/analysis/alphamax_extended_reference_20260913"
CRYPTO = ROOT / "artifacts/analysis/crypto_extended_reference_20260913"
VINTAGE = (
...
def inputs():
    ...
    for arm in ARMS:
        for source, csv_name, closure in [(MAX,'calendar2023_2026_excess.csv','closure.json'),(CRYPTO,'calendar2023_2026.csv','closure.json'),(TREND,'calendar2023_2026_excess.csv','audit_closure.json')]:
            paths.extend(source/arm/name for name in [csv_name,closure,'independent_audit.json','reservation.json','run/equity.parquet'])
...
def execute(arm):
    ...
    config = {
        "family": "combined_extended_reference",
        "arm": arm,
    ...
    reservation.update(
        ...
        family_trial_account="combined_extended_reference",
        return_identity_id=f"combined_extended_reference_{arm}_20260913",
        packet_public_path=f"/glassbox/trial-packets/combined-extended-reference-{arm}",
        paper_public_path="/research/combined-extended-reference-20260913",
    )
    ...
    for key,source in [('max',MAX),('trend',TREND)]:
        data = pd.read_csv(source/arm/'calendar2023_2026_excess.csv')
    ...
    crypto = pd.read_csv(CRYPTO/arm/'calendar2023_2026.csv')
