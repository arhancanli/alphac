...
R=Path(__file__).resolve().parents[1];O=R/'artifacts/analysis/combined_path_momentum_20260913/normal'
C=R/'artifacts/analysis/alphamax_path_momentum_20260913/candidate';B=R/'artifacts/analysis/bil_cash_20260913/normal'
S=R/'evidence/alphamax-path-momentum-design-20260913/DESIGN.json'
...
def main():
 ...
 config={'family':'alphamax_path_momentum_combined','arm':'normal','manifest_sha256':sha(O/'input_manifest.json'),'spec_sha256':sha(S)}
 log=ExperimentUnion.discover(O/'experiments.jsonl',R);now=datetime.now(timezone.utc)
 reservation=json.loads((B/'reservation.json').read_text());reservation.update(reserved_at=now.isoformat(),trial_config=config,hypothesis_identity=hypothesis_hash(config),family_trial_account='alphamax_path_momentum',return_identity_id='alphamax_path_momentum_combined_normal_20260913',packet_public_path='/glassbox/trial-packets/alphamax-path-momentum-combined-normal',paper_public_path='/research/alphamax-path-momentum-combined-normal')
