...
SPEC = ROOT / "evidence/combined-session-cooldown-20260913/EXPERIMENT_SPEC.json"
...
def source_arm(source, arm):
    return arm if source == MAX else arm.replace("candidate", "baseline")
...
def inputs():
    ...
    paths.append(ROOT/'evidence/alphamax-session-cooldown-20260913/EXPERIMENT_SPEC.json')
...
def execute(arm):
    ...
    config = {
        "family": "combined_session_cooldown",
        "arm": arm,
    ...
    reservation.update(
        ...
        family_trial_account="combined_session_cooldown",
        return_identity_id=f"combined_session_cooldown_{arm}_20260913",
        packet_public_path=f"/glassbox/trial-packets/combined-extended-reference-{arm}",
        paper_public_path="/research/combined-session-cooldown-20260913",
    )
