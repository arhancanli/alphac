...
R=Path(__file__).resolve().parents[1];O=R/'artifacts/analysis/combined_total_return_momentum_20260913/normal'
C=R/'artifacts/analysis/alphamax_total_return_momentum_20260913/candidate';B=R/'artifacts/analysis/bil_cash_20260913/normal'
S=R/'evidence/alphamax-dividend-momentum-feasibility-20260913/DESIGN.json'
...
def main():
 ...
 audit=json.loads((C/'independent_audit.json').read_text());assert audit['status']=='SAVED_MARKS_RECONCILED_SESSION_YEAR_VERIFIED'
 for p,h in audit['source_sha256'].items():assert sha(R/p)==h
 closure=json.loads((C/'closure.json').read_text());assert sha(R/closure['packet'])==closure['packet_sha256']
 spec=json.loads(S.read_text());paths=[S,C/'calendar2023_2026_excess.csv',C/'closure.json',C/'independent_audit.json',C/'feature_audit.json',C/'benchmark_audit.json',B/'daily.csv',B/'result.json',Path(__file__),R/'scripts/run_bil_cash_experiment.py',R/'pyproject.toml',R/'uv.lock']+list((R/'src/alphaforge').rglob('*.py'))
 write(O/'input_manifest.json',{'sha256':{str(p.relative_to(R)):sha(p) for p in paths}})
 config={'family':'alphamax_total_return_momentum_combined','arm':'normal','manifest_sha256':sha(O/'input_manifest.json'),'spec_sha256':sha(S)}
 log=ExperimentUnion.discover(O/'experiments.jsonl',R);now=datetime.now(timezone.utc)
 reservation=json.loads((B/'reservation.json').read_text());reservation.update(reserved_at=now.isoformat(),trial_config=config,hypothesis_identity=hypothesis_hash(config),family_trial_account='alphamax_total_return_momentum',return_identity_id='alphamax_total_return_momentum_combined_normal_20260913',packet_public_path='/glassbox/trial-packets/alphamax-total-return-momentum-combined-normal',paper_public_path='/research/alphamax-total-return-momentum-combined-normal')
 reservation['governance_epoch']['reservation_ordinal']=log.n_hypotheses()+1
 write(O/'preregistration.json',{'trial_config':config,'reserved_at':now.isoformat(),'gates':spec['combined_gates']})
 ...
 assert dates.dt.strftime('%Y-%m-%d').tolist()==pd.to_datetime(expected,unit='ms',utc=True).strftime('%Y-%m-%d').tolist()
 ...
 frame['max']=aligned.fillna(0).to_numpy()
 frame['total']=base.total+.225*(frame['max']-base['max']);frame['excess']=frame.total-frame.benchmark
