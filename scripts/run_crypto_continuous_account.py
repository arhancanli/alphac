...
DESIGN = ROOT / "evidence/crypto-continuous-account-20260913/EXPERIMENT_SPEC.json"
...
def prepare():
    ...
    files += [ROOT/"scripts/funded_inception_runner.py", ROOT/"scripts/continuous_crypto_runner.py", ROOT/"tests/unit/test_continuous_crypto_runner.py", ROOT/design["funded_policy"], ROOT/"artifacts/research/forecast_diagnostics/selection_debt_20260913.json"]
    verify(ROOT/design["funded_policy"],design["funded_policy_sha256"])
...
def preflight(arm_id):
    ...
    binding = {
        "scenario": arm,
        "continuous_accounting": design["continuous_accounting"],
        "initial_cash": scope["initial_cash"],
        "capital_inception": scope["capital_inception"],
        "funded_policy_sha256": design["funded_policy_sha256"],
...
def execute(arm_id):
    from continuous_crypto_runner import ContinuousCryptoRunner as WalkForwardRunner
    ...
    reservation.update(
        ...
        family_trial_account="crypto_continuous_account",
        return_identity_id=f"crypto_continuous_account_{arm_id}_20260913",
        packet_public_path=f"/glassbox/trial-packets/crypto-continuous-account-{arm_id}-20260913",
        paper_public_path=f"/research/crypto-continuous-account-{arm_id}-20260913",
    )
    ...
    class DurableTerminalBacktester(TerminalBacktester):
        ...
        def run(self, *args, **kwargs):
            ...
            scheduled = args[0] if args else kwargs["strategy"]
            result.config["model_boundary_transitions"] = scheduled.transitions
