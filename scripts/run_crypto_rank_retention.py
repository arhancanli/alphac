...
DESIGN = ROOT / "evidence/crypto-rank-retention-20260913/REPLAY_SPEC.json"
...
def prepare():
    ...
    files += [ROOT/'scripts/crypto_rank_retention.py', ROOT/'scripts/crypto_rank_retention_hook.py', ROOT/'tests/unit/test_crypto_rank_retention.py', ROOT/'tests/unit/test_crypto_rank_retention_hook.py', ROOT/'evidence/crypto-rank-retention-20260913/EXPERIMENT_SPEC.json']
...
def execute(arm_id):
    ...
    reservation.update(
        ...
        family_trial_account="crypto_rank_retention",
        return_identity_id=f"crypto_rank_retention_{arm_id}_20260913",
        packet_public_path=f"/glassbox/trial-packets/crypto-rank-retention-{arm_id}-20260913",
        paper_public_path=f"/research/crypto-rank-retention-{arm_id}-20260913",
    )
    ...
    class DurableTerminalBacktester(TerminalBacktester):
        def __init__(self, *args, **kwargs):
            ...
            kwargs['config_echo'] = {**kwargs['config_echo'], 'retention_buffer_ranks': arm['retention_buffer_ranks']}
            super().__init__(*args, **kwargs)

        def run(self, strategy, *args, **kwargs):
            from crypto_rank_retention_hook import install_rank_retention
            install_rank_retention(strategy, buffer_ranks=arm['retention_buffer_ranks'])
