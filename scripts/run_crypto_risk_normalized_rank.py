...
DESIGN = ROOT / "evidence/crypto-risk-normalized-rank-20260913/REPLAY_SPEC.json"
...
def prepare():
    ...
    files += [ROOT/'scripts/crypto_risk_normalized_rank.py',ROOT/'tests/unit/test_crypto_risk_normalized_rank.py',ROOT/'evidence/crypto-risk-normalized-rank-20260913/EXPERIMENT_SPEC.json']
...
def execute(arm_id):
    ...
    class DurableTerminalBacktester(TerminalBacktester):
        def __init__(self, *args, **kwargs):
            ...
            kwargs['config_echo']={**kwargs['config_echo'],'rank_score':arm['rank_score']}
            super().__init__(*args, **kwargs)

        def run(self, strategy, *args, **kwargs):
            from crypto_risk_normalized_rank import install_risk_normalized_rank
            install_risk_normalized_rank(strategy)
