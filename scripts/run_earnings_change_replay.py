...
OUT=ROOT/'artifacts/analysis/earnings_change_20260913'
...
def run(arm):
 ...
 if arm=='stress' and not (OUT/'normal/closure.json').exists():raise ValueError('Close normal accounting packet before stress')
 ...
 config=json.loads((OUT/'protocol.json').read_text())|{'cost_arm':arm,'allocator':'earnings_monthly_equal_notional','instrument_ids':ids,'input_manifest_sha256':sha(OUT/'input_manifest.json'),'resolved_costs':settings.costs.model_dump()}
 ...
 with InstrumentStore(directory/'ops.sqlite') as store:
  ...
  engine=PayableEquityBacktester(PITDataReader(LakePaths(SNAP/'lake')),store,TransactionCostModel.from_settings(settings),tf=Timeframe.D1,asset_class=AssetClass.EQUITY,payments=payments,retrospective_vintage_ms=schedule['retrospective_vintage_ms'],retrospective_actions=actions,no_trade_band_frac=.001,max_order_adv_frac=settings.risk.max_order_adv_frac,clamp_reduce_only_adv=settings.risk.clamp_reduce_only_adv,config_echo=config)
