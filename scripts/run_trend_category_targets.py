...
from alphaforge.portfolio.trend_category_targets import EQUITY_IDS, CategoryTargetsStrategy
...
OUT = ROOT / "artifacts/analysis/alphatrend_category_targets_20260913"
REPLAY = ROOT / "artifacts/analysis/alphatrend_positive_targets_20260913"
SAVED = ROOT / "artifacts/analysis/alphatrend_retrospective_comparison_20260912/baseline"
SPEC = ROOT / "evidence/alphatrend-category-targets-20260913/EXPERIMENT_SPEC.json"
...
class PositiveComparisonBundle(ConfirmationComparisonBundle):
    strategy_class = PositiveTargetsStrategy
...
class CategoryComparisonBundle(PositiveComparisonBundle):
    strategy_class = CategoryTargetsStrategy
...
def bundle_for(normalization, positive_targets):
    ...
    cls = CategoryComparisonBundle if positive_targets else PositiveComparisonBundle
    return cls(
...
def main():
    ...
    if ExperimentUnion.discover(OUT / "baseline/experiments.jsonl", ROOT).n_hypotheses() != 265:
        raise ValueError("Expected starting union 265")
    ...
    write(
        OUT / "protocol.json",
        {
            ...
            "baseline_replay_reason": (
                "Require confirmed control parity against259/260 before each candidate"
            ),
            "baseline": str(SAVED),
    ...
    for arm, positive_targets, stress in [
        ...
        config = {
            ...
            "category_targets": positive_targets,
            "cost_stress": stress,
            "category_targets_spec_sha256": sha(SPEC),
        ...
        reservation.update(
            ...
            family_trial_account="alphatrend_category_targets",
            return_identity_id=f"alphatrend_category_targets_{arm}_20260913",
            packet_public_path=f"/glassbox/trial-packets/alphatrend-category-targets-{arm}-20260913",
            paper_public_path=f"/research/alphatrend-category-targets-{arm}-20260913",
        )
        ...
        if positive_targets:
            assert all(
                w >= 0
                for row in strategy.direction_audit
                for iid, w in row["targets"].items()
                if iid in EQUITY_IDS
            )
    ...
    write(
        ...
        {
            "arms": results,
            "standalone_descriptive_only": True,
            "combined_gates_pending": True,
            "hypothesis_union": log.n_hypotheses(),
            "admitted": False,
        },
    )
