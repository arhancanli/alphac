"""Fixed registered direction-confirmation comparison; no parameter search or activation."""
...
OUT = ROOT / "artifacts/analysis/alphatrend_confirmation_20260913"
SAVED = ROOT / "artifacts/analysis/alphatrend_retrospective_comparison_20260912/baseline"
SPEC = ROOT / "evidence/alphatrend-direction-confirmation-20260913/EXPERIMENT_SPEC.json"
...
def bundle_for(normalization, confirmation):
    ...
    cls = ConfirmationComparisonBundle if confirmation else ComparisonBundle
    return cls(
...
def main():
    ...
    if ExperimentUnion.discover(OUT / "baseline/experiments.jsonl", ROOT).n_hypotheses() != 244:
        raise ValueError("Expected starting union 244")
    ...
    write(
        OUT / "protocol.json",
        {
            ...
            "baseline_replay_reason": (
                "Changed execution/paper.py and portfolio/book.py bindings; "
                "require equity parity before candidate"
            ),
            "baseline": str(SAVED),
    ...
    for arm, confirmation, stress in [
        ...
        directory = OUT / arm
        directory.mkdir()
        bundle = bundle_for(normalization, confirmation)
        ...
        config = {
            "allocator": "trend",
            "direction_confirmation": confirmation,
            "cost_stress": stress,
            "confirmation_spec_sha256": sha(SPEC),
        ...
        reservation.update(
            ...
            family_trial_account="alphatrend_direction_confirmation",
            return_identity_id=f"alphatrend_confirmation_{arm}_20260913",
            packet_public_path=f"/glassbox/trial-packets/alphatrend-confirmation-{arm}-20260913",
            paper_public_path=f"/research/alphatrend-confirmation-{arm}-20260913",
        )
    ...
    criteria = {
        "sharpe_gain_at_least_0_1": b["sharpe"] - a["sharpe"] >= 0.1,
        "retain_90pct_cagr": b["cagr"] >= 0.9 * a["cagr"],
        "no_worse_drawdown": b["max_dd"] <= a["max_dd"],
        "lower_turnover": b["turnover_ann"] < a["turnover_ann"],
        "stress_sharpe_improvement": results["candidate_stress"]["sharpe"]
