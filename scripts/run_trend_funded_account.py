...
SPEC = ROOT / "evidence/trend-funded-account-20260913/EXPERIMENT_SPEC.json"
...
START = 1672531200000  # January1 2023 funded cash inception
...
def main():
    ...
    parser.add_argument("--arm", choices=["baseline", "baseline_stress"], required=True)
    requested = parser.parse_args().arm
        ...
        write(
            OUT / "protocol.json",
            {
                ...
                "baseline_replay_reason": (
                    "22500fresh funded state fromJanuary1; no pre-inception positions"
                ),
                "baseline": str(SAVED),
    ...
    for arm, positive_targets, stress in [
        ...
        reservation.update(
            ...
            family_trial_account="confirmed_trend_funded_account",
            return_identity_id=f"trend_funded_account_{arm}_20260913",
            packet_public_path=f"/glassbox/trial-packets/trend-funded-account-{arm}-20260913",
            paper_public_path=f"/research/trend-funded-account-{arm}-20260913",
        )
        ...
        with InstrumentStore(directory / "ops.sqlite") as store:
            ...
            ).run(strategy, config["instrument_ids"], start=START, end=END, initial_cash=22500.0)
        ...
        write(directory/"execution_complete.json", {"status":"MEASURED_INDEPENDENT_ACCOUNTING_PENDING","qualified":False,"initial_cash":22500,"inception":START})
        results[arm] = metrics
