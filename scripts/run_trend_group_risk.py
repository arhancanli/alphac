"""Registered fixed-group allocator candidate; original controls retained."""
...
SPEC = ROOT / "evidence/trend-group-risk-design-20260913/EXPERIMENT_SPEC.json"
...
START = 1672272000000  # Dec29 2022 source session
END = 1780358400000  # June2 2026 exclusive
...
def bundle_for(normalization, positive_targets):
    ...
    assert not positive_targets
    cls = GroupComparisonBundle
    return cls(
...
def main():
    ...
    parser.add_argument("--arm", choices=["baseline", "baseline_stress"], required=False)
    parser.add_argument("--prepare", action="store_true")
    args = parser.parse_args()
    requested = args.arm
    if not args.prepare and not requested:
    ...
    files += [ROOT / name for name in [
        'scripts/trend_group_risk_allocator.py', 'scripts/trend_group_risk_strategy.py',
        'scripts/trend_group_risk_bundle.py', 'scripts/run_trend_extended_reference.py',
        'tests/unit/test_trend_group_risk_engine.py', 'tests/unit/test_trend_group_risk_allocator.py',
        'tests/unit/test_trend_group_risk_strategy.py']]
    ...
    if not (OUT / "input_manifest.json").exists():
        ...
        write(
            OUT / "protocol.json",
            {
                ...
                "baseline_replay_reason": (
                    "Fixed group-risk allocator candidate versus saved matched extended reference"
                ),
                "baseline": str(SAVED),
        ...
        previous = json.loads((OUT / "input_manifest.json").read_text())
        for path, digest in previous["bindings"].items():
            assert sha(ROOT / path) == digest
        assert json.loads((OUT / "protocol.json").read_text())["spec_sha256"] == sha(SPEC)
    if args.prepare:
        print("Prepared hash-bound group runner; no reservation or historical returns.")
        return
    results = {}
    for arm, positive_targets, stress in [
        ...
        config = {
            "allocator": "trend",
            "fixed_group_risk": True,
            "direction_confirmation": True,
            "positive_targets": positive_targets,
            "cost_stress": stress,
            "extended_reference_spec_sha256": sha(SPEC),
            "saved_signal_filter": "ts_open<end",
        ...
        reservation.update(
            ...
            family_trial_account="trend_fixed_asset_group_risk",
            return_identity_id=f"trend_group_risk_{arm}_20260913",
            packet_public_path=f"/glassbox/trial-packets/trend-group-risk-{arm}-20260913",
            paper_public_path=f"/research/trend-group-risk-{arm}-20260913",
        )
        ...
        with InstrumentStore(directory / "ops.sqlite") as store:
            ...
            signals = signals.loc[signals.index.get_level_values("ts_open") < END].copy()
        ...
        write(directory / "group_risk_audit.json", strategy.group_risk_audit)
        if positive_targets:
        ...
        write(directory / "measurement_status.json", {"status":"MEASURED_REQUIRES_INDEPENDENT_ACCOUNTING_AND_PACKET_CLOSURE", "qualified":False})
        results[arm] = metrics
        print(f"Completed {arm}: {json.dumps(metrics)}", flush=True)
    write(
        OUT / f"{requested}_execution_summary.json",
