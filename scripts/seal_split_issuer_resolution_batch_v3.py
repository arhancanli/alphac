#!/usr/bin/env python3
"""Seal the exact Ocean Rig restructuring reverse split without opening returns."""
...
PRICE: Final[Path] = (
    ...
    / "data/lake_sharadar/ohlcv_1d/instrument_id=XUSE:CASH:ORIGUSD/year=2017/data.parquet"
)
OUTPUT: Final[Path] = REPO / "artifacts/audit/split_issuer_resolution_batch_v3.json"
SOURCE: Final[dict[str, Any]] = {
    "ticker": "ORIG",
    "url": (
        "https://www.sec.gov/Archives/edgar/data/1447382/"
        "000091957417006882/d7659156_ex99-1.htm"
    ),
    "required_fragments": [
        "On September 21, 2017, the Company effected a 1-for-9,200 reverse stock split",
        "of the then-existing shares of the Company's issued common stock",
        "Following the reverse stock split, there were approximately 8,975 shares",
        "The CUSIP number for the common stock following the reverse stock split is G66964118",
    ],
    "instrument_id": "XUSE:CASH:ORIGUSD",
    "issuer_effective_date": "2017-09-21",
    "ex_date": "2017-09-22 04:00:00+04:00",
    "issuer_ratio": 1 / 9200,
    "ratio": 0.00011,
...
def _market_date_binding() -> dict[str, Any]:
    ...
    if len(rows) < 2 or rows.iloc[-1]["date"] != event_date:
        raise ValueError("missing ORIG event-date frozen price boundary")
    previous, current = rows.iloc[-2], rows.iloc[-1]
    if previous["date"].isoformat() != SOURCE["issuer_effective_date"]:
        raise ValueError("ORIG issuer effective date is not the final pre-event frozen bar")
    return {
...
def build(source: dict[str, Any], *, retrieved_at: str) -> dict[str, Any]:
    ...
    if float(source["issuer_ratio"]) != float(SOURCE["issuer_ratio"]):
        raise ValueError("ORIG issuer ratio does not match the sealed source specification")
    ...
    if len(matches) != 1:
        raise ValueError("ORIG evidence does not bind exactly one frozen unresolved event")
    event = matches[0]
    if event["lifecycle_classification"] != "WITHIN_PRICE_LIFECYCLE_REQUIRES_RESOLUTION":
        raise ValueError("ORIG row is no longer an in-lifecycle unresolved event")
    if round(float(source["issuer_ratio"]), 5) != float(event["stored_ratio"]):
        raise ValueError("ORIG issuer ratio does not match frozen five-decimal precision")
    verified = {
        **source,
    ...
    payload: dict[str, Any] = {
        "schema": "canli.alphac-split-issuer-resolution-batch.v3",
        "author": "Arhan Canli",
        "retrieved_at": retrieved_at,
        "decision": "ORIG_EXACT_RESTRUCTURING_REVERSE_SPLIT_ISSUER_VERIFIED",
        "hypotheses_spent": 0,
        ...
        "required_next_action": (
            "Merge only this exact instrument/date/stored-ratio tuple into the fail-closed "
            "policy. Concurrent restructuring issuance explains why the price boundary need not "
            "equal the reverse-split ratio."
        ),
        "claim_boundary": (
            "This seal verifies the old-share 1-for-9,200 mutation and its first frozen post-"
            "effective-date bar. It does not infer entitlement to restructuring issuances, open "
            "returns, authorize another event, or pass the global split gate."
        ),
    }
