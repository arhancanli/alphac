"""Isolated group allocator route; frozen paired rebalance body with one solve substitution."""
...
from alphaforge.portfolio.strategy import annualize_cov, ewma_cov, ledoit_wolf_cc, math, nearest_psd, np, vol_target
from trend_group_risk_allocator import GroupRiskAllocator


class GroupRiskPairedStrategy(PairedPriceBlendStrategy):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self._allocator_name != 'trend':
            raise ValueError('Group risk requires trend allocator settings')
        self._group_allocator = GroupRiskAllocator(self._allocator.constraints)
    ...
    def _rebalance(
        ...
        result = self._group_allocator.solve_for_ids(ids, mu, cov_ann, w_prev, cost, shortable)
...
class GroupRiskConfirmedStrategy(ConfirmedDirectionStrategy, GroupRiskPairedStrategy):
    """Inherited confirmation dispatches to group sizing through normal super/MRO."""
