...
s=r/'artifacts/analysis/alphatrend_positive_targets_20260913';c=r/'artifacts/analysis/alphac_positive_targets_20260913_attempt2'
...
for arm in arms:
 ...
 if arm.startswith('candidate'):
  assert (p.qty>=0).all() and meta['borrow_total']==0
  audit=json.loads((d/'direction_audit.json').read_text());assert all(w>=0 for row in audit for w in row['targets'].values())
