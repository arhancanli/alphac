"""Separate diagnostic feature, raw execution and raw label inputs.

This is an explicit input boundary, not an engine integration or full-history
certificate. Snapshot provenance and chronological collection remain separate gates.
"""
...
class TrendInputRoutes:
    def __init__(self, paired: pd.DataFrame):
        ...
        if (
            ...
            raise ObservationError("Positive finite raw volume required")
    ...
    def feature_bars(self, *, through_session: int) -> pd.DataFrame:
        return self._view("signal", through_session)
