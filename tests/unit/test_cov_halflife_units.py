...
from alphaforge.portfolio.strategy import LEGACY_COV_HALFLIFE_BARS, BlendStrategy
...
def _bars_for(days: float | None, calendar: object, tf: Timeframe) -> int:
...
def test_the_default_does_not_change_the_live_book() -> None:
    """The default must reproduce what every live sleeve is ACTUALLY running.

    No call site passes this argument -- the crypto loop (cli/paper_cmds.py), the AlphaTrend
    gauntlet and the AlphaMax walk-forward all take the default -- the walk-forwards are
    regenerated by the daily ticks, and live_cycle.py submits the last leg's weights straight to
    the broker. So a default here is not a configuration choice, it is a trade on every sleeve at
    once, and it was briefly set to the drawdown study's 21 days without that being measured on
    the live estimator. Opting in per profile is how that change should land.
    """
    default = inspect.signature(BlendStrategy.__init__).parameters["cov_halflife_days"].default
    assert default is None, (
        "a numeric default silently re-sizes every live sleeve on the next daily tick; pass "
        "cov_halflife_days explicitly at the profile that has measured it"
    )
    assert _bars_for(None, XNYSCalendar(), Timeframe.D1) == LEGACY_COV_HALFLIFE_BARS
    assert _bars_for(None, Always24x7Calendar(), Timeframe.H1) == LEGACY_COV_HALFLIFE_BARS
