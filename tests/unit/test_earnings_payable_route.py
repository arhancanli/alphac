...
def test_actual_engine_fills_first_month_source_open(tmp_path,revoke):
 ...
 with InstrumentStore(tmp_path/'ops.sqlite') as store:
  ...
  engine=PayableEquityBacktester(PITDataReader(paths),store,TransactionCostModel(),tf=Timeframe.D1,asset_class=AssetClass.EQUITY,cost_inputs=StaticCostInputs(adv_quote=1e9,sigma_daily=.02),payments=[])
